...
export function searchBuses(origin?: string, destination?: string) {
  if (USE_PG) return getPg().then((pg) => pg.searchBuses(origin, destination));
  return fileDb.searchBuses(origin, destination);
}

export function getNearbyBuses(latitude: number, longitude: number, radiusKm = 10) {
  if (USE_PG) return getPg().then((pg) => pg.getNearbyBuses(latitude, longitude, radiusKm));
  return fileDb.getNearbyBuses(latitude, longitude, radiusKm);
}

export function getSavedRoutes(userId: string) {
  if (USE_PG) return getPg().then((pg) => pg.getSavedRoutes(userId));
  return fileDb.getSavedRoutes(userId);
}

export function saveRoute(userId: string, routeId: string) {
  if (USE_PG) return getPg().then((pg) => pg.saveRoute(userId, routeId));
  return fileDb.saveRoute(userId, routeId);
}

export function unsaveRoute(userId: string, routeId: string) {
  if (USE_PG) return getPg().then((pg) => pg.unsaveRoute(userId, routeId));
  return fileDb.unsaveRoute(userId, routeId);
}

export function getFavouriteBuses(userId: string) {
  if (USE_PG) return getPg().then((pg) => pg.getFavouriteBuses(userId));
  return fileDb.getFavouriteBuses(userId);
}

export function favouriteBus(userId: string, busId: string) {
  if (USE_PG) return getPg().then((pg) => pg.favouriteBus(userId, busId));
  return fileDb.favouriteBus(userId, busId);
}

export function unfavouriteBus(userId: string, busId: string) {
